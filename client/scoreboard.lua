...
function ZEDBoard:Render( args )
	if(Key:IsDown(9))then
		...
		if(self.ViewPlayer > -1)then
			...
			Render:FillArea(Vector2(x -40,y), Vector2(Render.Width /5*1 - 10, 100 + (Render.Height - 200)), Scoreboard.BackgroundColor )
			local y2 = y+20
			local y = y2
			for _,ply in pairs(self.Players) do
				if(ply[1] == self.ViewPlayer)then
					for k,v in pairs(self.Header) do
						Render:DrawText( Vector2(x-Render:GetTextWidth(v .. ": ") + width/10,y), tostring(v .. ": "), Scoreboard.HeaderColor )
						Render:DrawText( Vector2(x + width/8,y), tostring(ply[k]), Scoreboard.HeaderColor )
						y = y + 20
					end
					for k,v in pairs(ply.ExtraInfo) do
						Render:DrawText( Vector2(x-Render:GetTextWidth(k .. ": ") + width/10,y), tostring(k .. ": "), Scoreboard.ExtraInfoColor )
						Render:DrawText( Vector2(x + width/8,y), tostring(v), Scoreboard.ExtraInfoColor )
						y = y + 20
					end
					break
				end
				...
				if(lply[1] == LocalPlayer:GetId())then
					for k,v in pairs(lply.Buttons) do
						if mpos.x > x -20 and mpos.x < x-20 + Render.Width / 5 -50 and mpos.y >= y and mpos.y <= y + 20 then
							Render:FillArea(Vector2(x -20,y), Vector2(Render.Width /5*1 - 50, 20), Scoreboard.ButtonSelectedColor )
						else
							Render:FillArea(Vector2(x -20,y), Vector2(Render.Width /5*1 - 50, 20), Scoreboard.ButtonColor )
						end
						Render:DrawText( Vector2(x-Render:GetTextWidth(k)/2 + width/3,y+3), tostring(k), Scoreboard.PlayerNameColor )
						y = y + 25
					end
					break
				end
		...
		Render:FillArea(Vector2(Render.Width/5-20 + modX,y), Vector2(Render.Width /5*3+40, 100 + (Render.Height - 200)), Scoreboard.BackgroundColor )
		Render:DrawText( Vector2(Render.Width/2 + modX - Render:GetTextWidth(self.ServerName, 20)/2,y+10), self.ServerName, Scoreboard.ServerNameColor, 20 )
		Render:DrawText( Vector2(Render.Width/2 + modX - Render:GetTextWidth("Players: " .. #self.Players .. "/" .. self.MaxPlayers .. " ("..round(100/self.MaxPlayers*#self.Players,2).."%)", 18)/2,y+30), "Players: " .. #self.Players .. "/" .. self.MaxPlayers .. " ("..round(100/self.MaxPlayers*#self.Players,2).."%)", Scoreboard.PlayerCountColor, 18 )
		...
		Render:FillArea(Vector2(Render.Width/5-20 + modX,Render.Height - 30), Vector2(Render.Width /5*3+40, 30), Scoreboard.BackgroundColor )
		Render:DrawText( Vector2(Render.Width/2-20 + modX,Render.Height - 20), tostring(credits), Scoreboard.ExtraInfoColor, 15 )
	end
end
